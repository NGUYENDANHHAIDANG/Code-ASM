...
namespace Drinkssss
{
    public partial class ManageCustomer : Form
    {
        ...
        public ManageCustomer(string authorityLevel, object employeeId1)
        {
            this.authorityLevel = authorityLevel;
...
private void FlushCustomerId()
        ...
        private void DeleteCustomer(int customerId)
        {
            ...
            if (connection != null)
            {
                ...
                // Declare SqlCommand to add parameters and execute query
                SqlCommand command = new SqlCommand(deleteOrderDetailQuery, connection);
                // add parameters
                command.Parameters.AddWithValue("customerId", customerId);
                // execute query (We do not need to know the result because this step is used to ensure no exception occur)
        ...
        private void SearchCustomer(string search)
        {
            ...
            else
            {
                ...
                if (connection != null)
                {
                    ...
                    // Initialize table
                    DataTable table = new DataTable();
                    // Fill the table with data queried from the database
                    adapter.Fill(table);
                    // Display the queried data preview by the dataset
                    dtgCustomer.DataSource = table;
                    // Close the connection
                    connection.Close();
                }
        ...
        private void gbCustomerManager_Enter(object sender, EventArgs e)
        {
